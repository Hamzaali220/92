...
@include('dashboard.include.sidebar')
		...
		<div class="container">
			...
			<div class="row margin-bottom-40">
				...
				<div class="col-md-9 ">
					<div class="padding-left-10 ">
						<h2 class="postdetailsh2 ">{{ $post->posttitle }} <small style="font-size: 55%;">{{$post->role_name}}</small> <span class="" style="font-size:11px; color:rgb(55, 160, 0);">{{ $post->applied_post == 1 ? ($post->applied_user_id == $user->id ? 'You are selected for this post.' : 'Post is selected.' ) : '' }}</span></h2>
						<div>
							<span class="margin-right-20"> <strong> Closing Date :</strong> {{ ($post->closing_date != null ) ? est_std_date($post->closing_date) : "Not updated yet" }} </span>
							<span class="margin-right-20"> <strong> Posted by:</strong> {{ ucfirst($post->name) }} <sub class="{{ $post->login_status }} mini"> {{ $post->login_status }}</sub></span>
							...
							<span class="margin-right-20"> Posted : <script type="text/javascript">document.write(timeDifference(new Date(), new Date(Date.fromISO('{{ $post->created_at }}'))));</script> </span>
							<span> <strong><i class="fa fa-map-marker"></i></strong> {{ ($post->city_name!= null)?$post->city_name.",":"" }}{{ ($post->state_name !== null)?$post->state_name.",":"" }}{{ $post->area!=null ? $post->area.',' : '' }} {{ $post->zip!=null ? $post->zip : '' }}</span>
						</div>
						<div>
							...
							@if($post->applied_post == 1 && $post->applied_user_id == $user->id && $post->closing_date != '' && $post->agent_payment == 'completed' && $post->buyer_seller_send_review == 1  && $post->agent_send_review == 1)
                            	<button class="btn-u margin-top-20 cursor"  data-toggle="modal"> Post has been closed</button>
							@endif

							@if($post->applied_post == 1 && $post->applied_user_id == $user->id && $post->closing_date != '' && $post->agent_payment != 'completed' && $agentPaymentStatus == true)

							@if(isset($commission_amount))
								<button class="btn-u margin-top-20 cursor" data-target="#make_payment" data-toggle="modal">Pay Now</button>
							@else
								<button class="btn-u margin-top-20 cursor" disabled="true" style="background: #aaa;">Pay Now</button>
							@endif
							...
							@if($post->buyer_seller_send_review == 1 && $post->applied_post == 1 && $post->applied_user_id == $user->id && $post->closing_date != '' && $post->agent_payment == 'completed' && $post->agent_send_review == 2 )
								<button class="btn-u margin-top-20 cursor" data-target="#set-agent-review" data-toggle="modal">Give a Review for post</button>
									...
									<div class="modal-dialog " style="display: grid;">
										<div class="modal-content not-top sky-form">
											...
											<div class="modal-header">
												...
												<h4 class="modal-title set-agent-review-title">Give a feedback to {{ $post->name }} for post ({{$post->posttitle}}) </h4>
											</div>
											...
											<div class="modal-footer" >
												...
												<input type="hidden" name="notification_message" value="{{ $userdetails->name }} has given you a review for post ({{ $post->posttitle }})">
												<input type="hidden" name="rating_type" value="4">
												<input type="hidden" name="rating_item_id" value="{{ $post->agents_user_id }}">
												...
												<button type="button" data-dismiss="modal" aria-hidden="true" class="btn-u" style="padding: 10px 20px;margin-right: 5px;color: #74c52c;box-shadow: 0 0.5px 4px rgba(57,73,76,.35);background: white;">Cancel</button>
			...
			<div class="row">
							...
							<div class="left-inner border1-bottom padding-5-20">
								...
								<ul class="list-unstyled">
									...
									@if($post->do_you_need_financing != null)
										<li><i class="fa fa-check color-green"></i>  need financing amount <strong> ${{ @number_format(str_replace('_',' ',$post->do_you_need_financing)) }} </strong> </li>
									@endif
								</ul>
							</div>
				...
				<div class="col-md-3 sm-margin-bottom-20 grid cs-style-5">
					...
					if(!empty($review)) {
						?>
							<div class=" margin-bottom-20" style="background-color: #fff;padding: 1.5rem;box-shadow: 0 1px 6px rgba(57, 73, 76, 0.52)">
					...
					<span class="tooltips cursor btn-block btn btn-default sitegreen" onclick="set_post_note_by_agent( '{{$post->agents_user_id}}', '{{ $post->post_id }}', '{{ $post->posttitle }}')" data-original-title="Note"><i class="fa fa-commenting bookmarkquestion-icon"></i> Note</span>
					...
					<button class="btn-block btn btn-default sitegreen margin-bottom-20"  data-toggle="modal" data-target="#askquestion" ><i class="fa fa-question-circle bookmarkquestion-icon"></i> Ask Question</button>
					<button class="btn-block btn btn-default sitegreen margin-bottom-20"  data-toggle="modal" data-target="#proposals-share"><i class="fa fa-file-text-o bookmarkquestion-icon"></i> Proposals </button>
					...
					<div class="tab-v1">
						...
						<div class="tab-content">
							...
							<div class="tab-pane fade" id="new-question">
								...
								<input type="hidden" value="{{ $post->agents_users_role_id }}" name="question_type" >
	...
	<div class="modal fade bs-example-modal-lg" id="proposals-share" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content" style="top: 0 !important;bottom: 0 !important;">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				...
				<div class="modal-body">
					...
					<div class="panel-body row" >
						...
						<div id="loadproposal" class="col-md-12 center loder loadproposal"><img src="{{ url('/assets/img/loder/loading.gif') }}" width="64px" height="64px"/></div>
							...
							<div class="modal-dialog " style="display: grid;">
								<div class="modal-content not-top sky-form">
										...
										<fieldset style="padding: 0px 15px;">
											...
											<section class="margin-0">
												...
												<label class="textarea margin-0">
													...
													<input type="text" class="form-control field-border datepicker" name="closing_date" value="" required onkeydown="return false" autocomplete="off" placeholder="Select closing date" />
									...
									<div class="modal-footer" >
										...
										<input type="hidden" name="notification_message" value="{{ isset($userdetails->name)?$userdetails->name:'' }} has given you a review for post ({{
										isset($post->posttitle)?$post->posttitle:'' }})">
										<input type="hidden" name="rating_type" value="3">
										<input type="hidden" name="rating_item_id" value="{{ isset($post->applied_user_id)?$post->applied_user_id:'' }}">
										<input type="hidden" name="rating_item_parent_id" value="{{ $post->post_id }}">
										<input type="hidden" name="receiver_id" value="{{ $post->agents_user_id }}">
										<input type="hidden" name="receiver_role" value="{{ $post->agents_users_role_id }}">
										<input type="hidden" name="sender_id" value="{{ $user->id }}">
				...
				<div class="modal-body">
					...
					<fieldset style="padding: 0px 15px;">
						...
						<section class="margin-0">
							...
							<label class="textarea margin-0">
								<input type="text" id="amount" name="amount" class="form-control" value="{{ (isset($commission_amount)?$commission_amount:'100') }}" readonly>
...
<script type="text/javascript">
	...
	$('.datepicker').datepicker({ dateFormat: "dd/mm/yy",maxDate: 'now', placeholder: "Select closing date"});
 ...
 //alert('{{ $uri_segment }}');
	...
	$( document ).ready(function() {
		...
		$('#message').click(function(e) {
			...
			publicConnection();
			localStorage.clear();
			window.location.href = "{{ URL('/messages/'.$post->post_id.'/'.$post->agents_user_id.'/'.$post->agents_users_role_id) }}";
		});

	   	$('#chat').click(function(e) {
	    	register_popup('{{$post->post_id}}','{{ $post->agents_user_id }}','{{ $post->agents_users_role_id }}');
	    	publicConnection();
		});
		...
		$.ajax({
			...
			type: 'POST',
			data: {shared_item_type : 1,_token : '{{ csrf_token() }}', add_by : '{{ $user->id }}', add_by_role : '{{ $user->agents_users_role_id }}', question_type : '{{ $post->agents_users_role_id }}', post_id : '{{ $post->post_id }}', receiver_id : '{{ $post->agents_user_id }}', share : 'ask_question' },
...
console.log('achi test');
								...
								'<div class="panel-group margin-0" id="accordion-'+utyp+'-'+key+'">'+
									...
									htm +='</span>';
											if(checkanswer != '' && checkanswer != null){
												htm +='<span class="text-13 margin cursor sitegreen pull-right accordion-toggle collapsed" data-toggle="collapse" data-target="#collapse-'+utyp+'-'+key+'" > <i class="fa fa-reply" aria-hidden="true"></i> <strong> Answered </strong> </span>';
											}
								htm +='</h4>';
										if(checkanswer != '' && checkanswer != null){
				...
				if('{{ $uri_segment }}'==='10'){
					...
					readnotificationbyreciverid('{{ $post->agents_user_id }}','{{ $post->agents_users_role_id }}','{{ $user->id }}','{{ $user->agents_users_role_id }}',10);
				}
	    ...
	    $('#enters-default-answers, #enters-default-question-answers').submit(function(e){
			...
			var $form = $(e.target),esmsg = $(".question-msg-"+question_id);
			...
			$.ajax({
				...
				data: {post_id : '{{ $post->post_id }}',notification_message : '{{ $userdetails->name }} replied ( '+answers+' ). your question ( '+qdd.question+' )' , receiver_id : '{{ $post->agents_user_id }}',receiver_role : '{{ $post->agents_users_role_id }}',question_id:question_id,answers:answers,_token : '{{ csrf_token() }}',from_id : '{{ $user->id }}',from_role : '{{ $user->agents_users_role_id }}'},
				beforeSend: function(){$(".loader-question-proccess_"+question_id).show(); $('.question-submit-b_'+question_id).hide(); },
				success: function(result) {
		...
		$.ajax({
			...
			type: 'POST',
			data: {shared_type:1,shared_item_type:2,shared_item_type_id:'{{ $post->post_id }}',_token : '{{ csrf_token() }}',receiver_id: '{{ $user->id }}',receiver_role : '{{ $user->agents_users_role_id }}',question_type : '{{ $user->agents_users_role_id }}',sender_id : '{{ $post->agents_user_id }}',sender_role : '{{ $post->agents_users_role_id }}',bookmark_type : 1,rating : 'rating'},
			...
			console.log(result[0]);
				...
				if(result[0] != ''){
					...
					$.each( result[0], function( key, val ) {
						...
						var rating1 = '';
						var ratingset=0;
						if(result[3][val.question_id] !=null && result[3][val.question_id] != 'undefined' && result[3][val.question_id] !=''){

							var rat = result[3][val.question_id];
							var ratingset = rat.rating;

							rating1 = 	'<div id="ratinganswer_'+val.question_id+'" class="rating-show-only anserrating">'+

					                        '<input class="stars" disabled type="radio"  id="star5_'+val.question_id+'" value="5" />'+
					                        '<label class = "full tooltips" data-original-title="Awesome " data-placement="top" for="star5_'+val.question_id+'" title="Awesome"></label>'+

					                        '<input class="stars" disabled type="radio" id="star4_5_'+val.question_id+'" value="4_5" />'+
					                        '<label class="half tooltips" data-original-title="Pretty good " data-placement="top" for="star4_5_'+val.question_id+'" title="Pretty good"></label>'+

					                        '<input class="stars" disabled type="radio" id="star4_'+val.question_id+'" value="4" />'+
					                        '<label class = "full tooltips" data-original-title="Pretty good " data-placement="top" for="star4_'+val.question_id+'" title="Pretty good"></label>'+

					                        '<input class="stars" disabled type="radio" id="star3_5_'+val.question_id+'" value="3_5" />'+
					                        '<label class="half tooltips" data-original-title="Meh " data-placement="top" for="star3_5_'+val.question_id+'" title="Meh"></label>'+

					                        '<input class="stars" disabled type="radio" id="star3_'+val.question_id+'" value="3" />'+
					                        '<label class = "full tooltips" data-original-title="Meh " data-placement="top" for="star3_'+val.question_id+'" title="Meh"></label>'+

					                        '<input class="stars" disabled type="radio" id="star2_5_'+val.question_id+'" value="2_5" />'+
					                        '<label class="half tooltips" data-original-title="Kinda bad " data-placement="top" for="star2_5_'+val.question_id+'" title="Kinda bad "></label>'+

					                        '<input class="stars" disabled type="radio" id="star2_'+val.question_id+'" value="2" />'+
					                        '<label class = "full tooltips" data-original-title="Kinda bad " data-placement="top" for="star2_'+val.question_id+'" title="Kinda bad"></label>'+

					                        '<input class="stars" disabled type="radio" id="star1_5_'+val.question_id+'" value="1_5" />'+
					                        '<label class="half tooltips" data-original-title="Meh " data-placement="top" for="star1_5_'+val.question_id+'" title="Meh"></label>'+

					                        '<input class="stars" disabled type="radio" id="star1_'+val.question_id+'" value="1" />'+
					                        '<label class = "full tooltips" data-original-title="Sucks big time " data-placement="top" for="star1_'+val.question_id+'" title="Sucks big time"></label>'+

					                        '<input class="stars" disabled type="radio"  id="star0_5_'+val.question_id+'" value="0_5" />'+
					                        '<label class="half tooltips" data-original-title="Sucks big time " data-placement="top" for="star0_5_'+val.question_id+'" title="Sucks big time"></label>'+
					                    '</div>';
						...
						var htm =
								'<div class="panel-group margin-0"><div class="panel-heading border1-bottom clear-both">'+
									...
									'<h4 class="text-15 panel-title question-title">'+
										'<span onclick="setnotesinaskedquestion('+val.question_id+',{{ $post->post_id }});" data-toggle="tooltip" data-original-title="Note" data-placement="top"  class="text-13 tooltips fa fa-commenting bookmarkquestion-icon"></span>'+
										'<span class="text-13 bookmark_q_'+val.question_id+'">';

										if(result[2][val.question_id] !=''){
											var book = result[2][val.question_id];
											htm +='<span onclick="remove_bookmark_list('+val.question_id+','+book.bookmark_id+');" data-toggle="tooltip" data-original-title="Bookmarked " class="tooltips fa fa-bookmark sitegreen book_question_'+val.question_id+' bookmarkquestion-icon"></span> ';
										}else{

											htm +='<span onclick="add_bookmark_list('+val.question_id+');" data-toggle="tooltip" data-original-title="Bookmark" class="tooltips fa fa-bookmark-o book_question_'+val.question_id+' bookmarkquestion-icon"></span> ';
										}
								htm +='</span>'+
										'<span>'+
										(key+1)+') </span><a class="question-question q-s-'+val.question_id+'" >'+
										 	...
										 	'</a>'+
									 	'<span href="#" class="text-13 margin cursor sitegreen pull-right accordion-toggle collapsed" data-toggle="collapse" data-target="#collapse-answer-'+key+'"> <i class="fa fa-reply"> </i> <small> Answer </small> </span>'+
								 	'</h4>'+

								 	'<div id="collapse-answer-'+key+'" class="panel-collapse collapse panel-body padding-0 margin-top-10">'+

										'{!! Form::open(array("url" => "#","class"=>"row enters-default-answers")) !!}'+
											'<div class="hide question-msg-'+val.question_id+'"></div>'+
											'<div class="col-md-8">'+
												'<label class="textarea ">'+
													'<textarea data-original-title="Enter your interesting answer" rows="2" class="field-border tooltips" name="question_default_'+key+'" id="question_default_'+key+'" placeholder="Enter your interesting answer">'+result[1][val.question_id]+'</textarea>'+
											...
											'<div class="col-md-4 margin-top-5"> '+
												...
												'<img class="ovloader loader-question-proccess_'+val.question_id+' pull-right" src="{{ url("/assets/img/loder/loading.gif") }}" width="15px" height="15px">'+
												'<button type="submit" class="question-submit-b_'+val.question_id+' btn-u ladda-button pull-right">Save</button><br>'+
												rating1+
											'</div>'+
						...
						$('#star'+ratingset+'_'+val.question_id).attr("checked", "checked");
					...
					if('{{ $uri_segment }}'==='4' || '{{ $uri_segment }}'==='8'){
						...
						readnotificationbyreciverid('{{ $post->agents_user_id }}','{{ $post->agents_users_role_id }}','{{ $user->id }}','{{ $user->agents_users_role_id }}',4);
						readnotificationbyreciverid('{{ $post->agents_user_id }}','{{ $post->agents_users_role_id }}','{{ $user->id }}','{{ $user->agents_users_role_id }}',8);
					}
		...
		$.ajax({
			...
			type: 'POST',
			data: {shared_type:2,shared_item_type:2,shared_item_type_id:'{{ $post->post_id }}', receiver_id: '{{ $user->id }}',receiver_role : '{{ $user->agents_users_role_id }}',sender_id : '{{ $post->agents_user_id }}',sender_role : '{{ $post->agents_users_role_id }}',_token : '{{ csrf_token() }}'},
			...
			success: function(result) {
				if(result != 'undefined' && result  != null && result !=''){
					...
					if('{{ $uri_segment }}'==='5' ){
						...
						readnotificationbyreciverid('{{ $post->agents_user_id }}','{{ $post->agents_users_role_id }}','{{ $user->id }}','{{ $user->agents_users_role_id }}',5);
					}
		...
		$.ajax({
			...
			success: function(result) {
				...
				if(result.bookmark_id != null && result.bookmark_id != 'undefined'){
					...
					var html ='<span class="tooltips cursor btn-block btn btn-default sitegreen" onclick="user_add_bookmark_list();" data-original-title="Bookmark"><i class="fa fa-star-o bookmarkquestion-icon"></i> Bookmark Post</span>';
	...
	function add_ask_list_question(id) {
		...
		var postque = agent_question[id];
       	$.ajax({
			...
			type: 'post',
			data: {notification_type : 1,notification_message : '{{ $userdetails->name }} asked questions related to your post `{{ $post->posttitle }}`' ,shared_type:1 ,shared_item_id:id,shared_item_type : 1,shared_item_type_id : '{{ $post->post_id }}',receiver_id : '{{ $post->agents_user_id }}',receiver_role: '{{ $post->agents_users_role_id }}',sender_id: '{{ $user->id }}',sender_role : '{{ $user->agents_users_role_id }}',_token : '{{ csrf_token() }}'},
	...
	function add_ask_list_question_remove(id,shared_id) {
		var postque = agent_question[id];
       	$.ajax({
			...
			success: function(result) {
				$('.imgloader_'+id).hide();$('.clicksurvey_'+id+' span').show();
				$('.clicksurvey_'+id).html('<span href="#" class="margin cursor pull-right" onclick="add_ask_list_question('+id+');"> <i class="fa fa-circle-o" aria-hidden="true"></i> <strong>Ask</strong> </span>');
	...
	function publicConnection() {
       	$.ajax({
			...
			type: 'post',
			data: { post_id : '{{ $post->post_id }}', from_id : '{{ $post->agents_user_id }}',from_role: '{{ $post->agents_users_role_id }}',to_id: '{{ $user->id }}',to_role : '{{ $user->agents_users_role_id }}',_token : '{{ csrf_token() }}' },
			success: function(result) {
			},error: function(result) {
			}
		});
	...
	function loadproposal(limit) {
		...
		$.ajax({
			...
			type: 'post',
			data: {shared_item_type : 1,_token : '{{ csrf_token() }}', sender_id : '{{ $user->id }}', sender_role : '{{ $user->agents_users_role_id }}', receiver_id : '{{ $post->agents_user_id }}' , receiver_role : '{{ $post->agents_users_role_id }}', post_id : '{{ $post->post_id }}' },
			beforeSend: function(){$(".loadproposal").show();},
    	    processData:true,
			success: function(result) {
				...
				if(proppos.count !== 0){
					...
					$.each( proppos.result, function( key, value ) {
						...
						if(shareproposals !=null && shareproposals != 'undefined' && shareproposals){
							...
							var asrvfun ='<i onclick="shareproposalremove('+value.proposals_id+','+shareproposals.shared_id+')"  aria-hidden="true" class="icon-custom rounded-x icon-line fa fa-times proposal-icon"></i>';
						}else{
							var classactive = '';
							var asrvfun ='<i onclick="shareproposal('+value.proposals_id+')"  aria-hidden="true" class="icon-custom rounded-x icon-line fa fa-share-alt proposal-icon"></i>';
						}
						var htmll = '<div class="col-md-4">'+
										'<div class="thumbnails '+classactive+' proposal_'+value.proposals_id+' thumbnail-style box-shdow thumbnail-kenburn">'+
											'<span class="proposal_share_'+value.proposals_id+'">'+asrvfun+'</span>'+
											'<div class="cbp-caption thumbnail-img">'+
												'<div class="overflow-hidden cbp-caption-defaultWrap">';
										...
										htmll +='</div>'+
												'<div class="removehover cbp-caption-activeWrap">'+
													'<div class="cbp-l-caption-alignCenter">'+
														'<div class="cbp-l-caption-body">'+
															'<ul class="link-captions no-bottom-space">'+
																'<li><a class="cursor" onclick="openpropop('+value.proposals_id+')"><i class="rounded-x fa fa-search"></i></a>'+
															'</ul>'+
														'</div>'+
													'</div>'+
												'</div>'+
	...
	function shareproposal(id) {
		...
		$('.proposal_'+id).addClass('proposal-active');
       	$.ajax({
			url: "{{url('/shared/data/insert')}}",
			type: 'post',
			data: {notification_type : 3,notification_message : '{{ $userdetails->name }} share a proposal related to your post `{{ $post->posttitle }}`' ,shared_type:3 ,shared_item_id:id,shared_item_type : 1,shared_item_type_id : '{{ $post->post_id }}',receiver_id : '{{ $post->agents_user_id }}',receiver_role: '{{ $post->agents_users_role_id }}',sender_id: '{{ $user->id }}',sender_role : '{{ $user->agents_users_role_id }}',_token : '{{ csrf_token() }}'},
			success: function(result) {
				$('.proposal_share_'+id).html('<i onclick="shareproposalremove('+id+','+result.data+')"  aria-hidden="true" class="icon-custom rounded-x icon-line fa fa-times proposal-icon"></i>');
				msgshowfewsecond('This proposal successfully shared.');
			},error: function(result) {
	...
	function shareproposalremove(id,shared_id) {
		...
		$('.proposal_'+id).removeClass('proposal-active');
       	$.ajax({
			...
			data: { id : id, shared_id : shared_id, _token : '{{ csrf_token() }}' },
			success: function(result) {
				$('.proposal_share_'+id).html('<i onclick="shareproposal('+id+')"  aria-hidden="true" class="icon-custom rounded-x icon-line fa fa-share-alt proposal-icon"></i>');
	...
	function shareuploadfiles(id) {
		...
		$('.uploadfiles_'+id).addClass('proposal-active');
       	$.ajax({
			url: "{{url('/shared/data/insert')}}",
			type: 'post',
			data: {notification_type : 2,notification_message : '{{ $userdetails->name }} share a files related to your post `{{ $post->posttitle }}`' ,shared_type:2 ,shared_item_id:id,shared_item_type : 1,shared_item_type_id : '{{ $post->post_id }}',receiver_id : '{{ $post->agents_user_id }}',receiver_role: '{{ $post->agents_users_role_id }}',sender_id: '{{ $user->id }}',sender_role : '{{ $user->agents_users_role_id }}',_token : '{{ csrf_token() }}'},
			success: function(result) {
	...
	function add_bookmark_list(id) {
		...
		var qq = sharedquestion_data[id];
       	$.ajax({
			...
			type: 'post',
			data: {bookmark_type : 1,bookmark_item_id : id, bookmark_item_parent_id : '{{ $post->post_id }}',receiver_id : '{{ $post->agents_user_id }}',receiver_role: '{{ $post->agents_users_role_id }}',sender_id: '{{ $user->id }}',sender_role : '{{ $user->agents_users_role_id }}',_token : '{{ csrf_token() }}'},
			success: function(result) {
				$('.bookmark_q_'+id).html('<span onclick="remove_bookmark_list('+id+','+result.data+');" data-toggle="tooltip" data-original-title="Bookmarked" class="tooltips fa fa-bookmark sitegreen book_question_'+id+' bookmarkquestion-icon"></span> ');
	...
	function remove_bookmark_list(id,bookmark_id) {
		var qq = sharedquestion_data[id];
       	$.ajax({
			...
			type: 'get',
			success: function(result) {
				$('.bookmark_q_'+id).html('<span onclick="add_bookmark_list('+id+');" data-toggle="tooltip" data-original-title="Bookmark" class="tooltips fa fa-bookmark-o sitegreen book_question_'+id+' bookmarkquestion-icon"></span> ');
	...
	function user_add_bookmark_list() {
       	$.ajax({
			url: "{{url('/bookmark/data/insert')}}",
			type: 'post',
			data: {bookmark_type : 2,bookmark_item_id :'{{ $post->agents_user_id }}', bookmark_item_parent_id : '{{ $post->post_id }}',receiver_id : '{{ $post->agents_user_id }}',receiver_role: '{{ $post->agents_users_role_id }}',sender_id: '{{ $user->id }}',sender_role : '{{ $user->agents_users_role_id }}',_token : '{{ csrf_token() }}'},
			success: function(result) {
	...
	function user_remove_bookmark_list(bookmark_id) {
       	$.ajax({
			...
			type: 'get',
			success: function(result) {
				$('.bookmark-user').html('<span class="tooltips cursor btn-block btn btn-default sitegreen" onclick="user_add_bookmark_list();" data-original-title="Bookmark"><i class="fa fa-star-o bookmarkquestion-icon"></i> Bookmark Post</span>');
	...
	$(function() {
	  ...
	  function stripeResponseHandler(status, response) {
	    if (response.error) {
	      ...
	      $.ajax({
				...
				type: 'POST',
				data: $form.serialize(),
				beforeSend: function(){$("#set-payment-loader").show();},
	    	    processData:false,
				success: function(result) {
					...
					if(typeof result.error !='undefined' && result.error !=null){
					 	...
					 	$.each( result.error, function( key, value ) {
					 	 $('#'+key+'_error').removeClass('hide').addClass('show').text(value);
						});
					}
					if(typeof result.normalerror !='undefined' && result.normalerror !=null){

					 	$('.error').removeClass('hide').find('.alert').text(result.normalerror);
					}
					if(typeof result.msg !='undefined' && result.msg !=null){
						$('.error').removeClass('hide').find('.alert').removeClass('alert-danger').addClass('alert alert-success').html(result.msg);
			  	...
			  	error: function(data)
		    	{
		    		...
		    		if(data.status=='500'){
						...
						$('.error').removeClass('hide').find('.alert').text(data.responseJSON.image[0]);
		    		}
		    	}
	  ...
	  $('#set_closing_date').submit(function(e){
	  		...
	  		if(count > 0) {
	  			...
	  			count++;
	  			$('.closing_date_warning').html('Once you save the closing date, you won\'t be able to change it later');
	  			$('#closingbtn').html('Confirm <i class="fa fa-check-circle-o" aria-hidden="true"></i>');
	  			return false;
	  		}
			...
			if((closing_date.getTime() >= post_create_date.getTime()) == false){
				$('.closing_date_warning').html('You can not set closing date before the post was created');
	  			return false;
			}
