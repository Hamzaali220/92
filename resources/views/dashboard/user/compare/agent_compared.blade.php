	...
	<div class="container content">
		...
		<div class="row pricing-table-v1 box-shadow-profile  no-space-pricing table-responsive">
			<table class="table table-bordered">
				<thead>
					<tr>
						<th class=""><div class="col-md-12">Compared items</div></th>
							...
							@foreach($result['result'] as $agentsdata)
								<?php 
									...
									if($agentsdata->applied_post == 1 && $agentsdata->applied_user_id == $agentsdata->details_id){
										...
										$title = 'Selected this agents for post ('.$agentsdata->posttitle.')';
									}
								 ?>
								<th class=" {{ $selectedclass }} agents_{{ $agentsdata->details_id }}" title="{{ $title }}">
		                            <div class="col-md-10"><img class="compare-property-img rounded-x" width="40" height="40" src="@if($agentsdata->photo != null && $agentsdata->photo != '') {{ url('assets/img/profile/') }}/{{ $agentsdata->photo }} @else {{ URL::asset('assets/img/testimonials/user.jpg') }} @endif" alt="" title=""> <b>{{ ucwords(strtolower($agentsdata->name)) }}</b>  </div>
		                            <div class="col-md-2">
					...
					<tbody class="list-unstyled pricing-v1-content">
						...
						@if($sataging['asked_question'] == 1 && $sataging['asked_question_list'] !='' && !empty($result['count']))
							@php $a=1 @endphp
							@foreach($sataging['asked_question_text'] as $key => $asked_question_text)
								<tr>
									...
									@foreach($result['result'] as $agentsdata)
										<td><li> <strong class="hidetext2line popover-compare-hover" >Ans.</strong> {{ $agentsdata->asked_question[$key]->answers != '' ? $agentsdata->asked_question[$key]->answers : '' }}</li></td>
									@endforeach
								</tr>
							@php ++$a @endphp
							@endforeach
						@endif
						@if($sataging['bookmark_agents'] == 1 && !empty($result['count']))
							<tr>
								<td><li> <strong> Bookmark </strong></li></td>
								@foreach($result['result'] as $agentsdata)
								<td><li> {{ $agentsdata->bookmark_agents != '' ? 'Yes' : 'No' }}</li></td>
								@endforeach
							</tr>
						@endif
						@if($sataging['bookmark_answers'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['bookmark_messages'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['bookmark_proposal'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['rating_answers'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['rating_messages'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['notes_agents'] == 1 && !empty($result['count']))
							<tr>
								<td><li> <strong> Notes </strong></li></td>
								@foreach($result['result'] as $agentsdata)
								<td><li> {!! !empty($agentsdata->notes_agents) && $agentsdata->notes_agents != '' ? $agentsdata->notes_agents->notes : 'No' !!}</li></td>
								@endforeach
							</tr>
						@endif
						@if($sataging['notes_asked_question'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['notes_answers'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['notes_messages'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['notes_proposal'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['proposals'] == 1 && !empty($result['count']))
							<tr>
						...
						@endif
						@if($sataging['documents'] == 1 && !empty($result['count']))
							<tr>
						...
						<tr>
							...
							<td>
								<li> 
									...
									<div class="left-inner  padding-5">
										<ul class="timeline-v2 timeline-me" id="">
											...
											<li>
												...
												<div class="cbp_tmlabel">
													<h5><b>{{$industry->organization}}</b></h5>
