        ...
        <div class="col-md-3">
          ...
          <div class="box box-primary">
            <div class="box-body box-profile">
              <img class="profile-user-img img-responsive img-circle" src="@if($userdetails->photo != '') {{ url('/assets/img/profile/'.$userdetails->photo) }} @else {{ url('/assets/img/team/img32-md.jpg') }} @endif" alt="User profile picture">
            ...
            <div class="tab-content">
                      ...
                      <span class="text-muted">
                        @if(!empty($userdetails->skills))
                          @foreach($userdetails->skills as $skills)
                            <span skey="{{$skills->skill_id}}" class="label label-success" >{{$skills->skill}}</span>
                          @endforeach
                        @endif
                    </span>
                   </td>
                  ...
                  <!-- The timeline -->
                    <ul class="timeline timeline-inverse">
                      ...
                      @foreach(json_decode($userdetails->employment) as $employment)
                        <li class="time-label">
                          <span class="bg-green">
                           {{$employment->post}} . {{  str_replace('_',' ',$employment->from) }} - {{  str_replace('_',' ',$employment->to) }}
                          </span>
                        </li>
                        <li>
                          <i class="fa fa-certificate bg-purple"></i>
                          <div class="timeline-item">
                            <h3 class="timeline-header">{{$employment->organization}}</h3>
                            <div class="timeline-body">
              ...
              <div class=" tab-pane" id="Personal">
                ...
                <tr>
                   ...
                   <tr>
                    <td>
                      <strong> Statement document  <br> <small>Physically sign that pdf </small></strong>
                    </td>
                    <td>
                        ...
                        <ul class="mailbox-attachments clearfix">
                          <li>
                            <span class="mailbox-attachment-icon has-img">
                              <iframe src="https://docs.google.com/viewer?url={{ $userdetails->statement_document }}&embedded=true" frameborder="0" scrolling="no" width="198" height="132"></iframe>
                            </span>
                            <div class="text-center">
                                <a href="#" onclick="showdocs('https://docs.google.com/viewer?url={{ $userdetails->statement_document }}&embedded=true','doc');" class="btn-u btn-block margin-bottom-20"> View</a>
                  ...
                  <td>
                    <ul class="timeline timeline-inverse">
                      ...
                      @foreach(json_decode($userdetails->real_estate_education) as $real_estate_education)
                        <li class="time-label">
                          <span class="bg-red">
                           {{$real_estate_education->degree}} . {{  str_replace('_',' ',$real_estate_education->from) }} - {{  str_replace('_',' ',$real_estate_education->to) }}
                          </span>
                        </li>
                        <li>
                          <i class="fa fa-graduation-cap bg-purple"></i>
                          <div class="timeline-item">
                            <h3 class="timeline-header">{{$real_estate_education->school}}</h3>
                            <div class="timeline-body">
                  ...
                  <td>
                    <span class="text-muted">
                      ...
                      @foreach($userdetails->certifications as $certifications)
                        <span skey="{{$certifications->certifications_id}}">{{$certifications->certifications_description}},</span>
                      @endforeach
                      @endif
                    </span>
                  </td>
                    ...
                    <td>
                     <ul class="timeline timeline-inverse">
                      ...
                      @foreach(json_decode($userdetails->industry_experience) as $industry_experience)
                        <li class="time-label">
                          <span class="bg-green">
                           {{$industry_experience->post}} . {{  str_replace('_',' ',$industry_experience->from) }} - {{  str_replace('_',' ',$industry_experience->to) }}
                          </span>
                        </li>
                        <li>
                          <i class="fa fa-certificate bg-purple"></i>
                          <div class="timeline-item">
                            <h3 class="timeline-header">{{$industry_experience->organization}}</h3>
                            <div class="timeline-body">
                                ...
                                <li>
                                  ...
                                  <div class="timeline-item">
                                    <table class="table border">
                                      ...
                                      <tr>
                                        <td>{{$sales_history->sellers_represented}}</td>
                                        <td>{{$sales_history->buyers_represented}}</td>
                                        <td>{{$sales_history->total_dollar_sales}}</td>
                                      </tr>
                                    </table>
                                  </div>
                                </li>
                  ...
                  <div class="tab-content">
                    ...
                    <div class="profile-edit tab-pane fade in active" id="default-answers">
                      <div id="enters-default-answers" class="sky-form">
                        <header> <i class="fa fa-question" aria-hidden="true"> </i> Admin Questions with Answers</header>
                    ...
                    <div class="profile-edit tab-pane fade " id="questions-ask">
                      <div id="enters-questions-to-ask" class="sky-form">
                        <header> <i class="fa fa-question-circle" aria-hidden="true"></i> {{ $userdetails->name }} uploaded questions For Buyer/Seller </header>
                        <br>
                        <div class="tab-v1">
                          <ul class="nav nav-tabs">
...
<script type="text/javascript">
  ...
  function loadproposal(limit) {
    ...
    $.ajax({
      url: "{{url('/agentadmin/')}}/agent/proposal/get/ten/"+limit+"/{{ $user->id }}/{{ $user->agents_users_role_id }}",
      type: 'get',
        ...
        if(result.count !== 0){
          ...
          $.each( result.result, function( key, value ) {
            ...
            if(extension=='png' || extension=='jpg' || extension=='jpeg' || extension=='gif' || extension=='tif'){
              docep   ='<img    class="documen document_'+value.proposals_id+'" src="'+value.proposals_attachments+'" frameborder="0" scrolling="no" width="225" height="142">';
            }else{
            ...
            var htmll = '<div class="col-md-4">'+
                    '<div class="thumbnails thumbnail-style thumbnail-kenburn">'+
                      ...
                      '<div class="caption cbp-title-dark">'+
                        '<span class="cbp-l-grid-agency-title"><a class="cursor hover-effect" onclick="openpropop('+value.proposals_id+')"   title="'+value.proposals_attachments+'"><strong>'+value.proposals_title+'</strong></a></span>'+
                      '</div>'+
                    '</div>'+
                  '</div>';
  ...
  function openpropop(el) {
    ...
    var docs='';
      if(extension=='png' || extension=='jpg' || extension=='jpeg' || extension=='gif' || extension=='tif'){
        docs ='<img     class="documen document_'+proposale_data[el].proposals_id+'" src="'+proposale_data[el].proposals_attachments+'" frameborder="0" scrolling="no"  width="100%" height="300">';
      }else{
        docs ='<iframe  class="documen document_'+proposale_data[el].proposals_id+'" src="https://docs.google.com/viewer?url='+proposale_data[el].proposals_attachments+'&embedded=true" frameborder="0" scrolling="no"  width="100%" height="300"></iframe>';
      }
